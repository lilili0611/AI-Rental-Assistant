...
对接飞书数据、面向客户与内部员工的 AI 租赁管理助手。当前 **v2.9.4** 已实现四类咨询路由、导购发散问题暂停与恢复，并将收货地址纳入下单确认与订单信息。

线上地址：[https://bozipaopao.cn/](https://bozipaopao.cn/)（Vercel 托管，当前版本 v2.9.4）。
...
覆盖 Spec §9/§13–§16/§20 关键路径：按日期库存、价格边界、预留释放、订单状态机、乐观锁、人工收款、收货地址校验与权限、知识库优先、四类咨询路由、LLM 180 字硬上限、多轮导购、免押与下单带入、陪伴提醒、安全排障和损坏标准图。
