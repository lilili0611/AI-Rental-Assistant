...
| 版本 | v2.7 |
| 状态 | Phase 1+2 与 v2.1–v2.7 已实现；v2.7 为知识库优先的猫猫头导购能力 |
| 配套文档 | 《产品需求文档 (PRD) v2.7》 |
...
1. **不合理请求检测**：命中后返回 `ai_response="请咨询人工"`、`answer_source="human"`，并提供转人工 action。
...
4. **LLM 兜底**：前述路径都无法回答时生成通用导购建议。成功时 `answer_source="llm"`；不可用或失败时转人工。
...
- 若模型返回“请咨询人工”、空内容或发生异常，最终响应必须是精确文本“请咨询人工”，且不添加 AI 标记。
...
```json
{
  "answer_source": "knowledge_base | business_data | llm | human"
}
```
...
4. 不合理请求、LLM 未配置、超时和异常均精确回复“请咨询人工”。
